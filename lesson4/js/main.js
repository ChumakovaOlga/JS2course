...
const validateFields = function() {
    let nameField = document.getElementById('form__input-name');
    let phoneField = document.getElementById('form__input-phone');
    let emailField = document.getElementById('form__input-email');
    let textField = document.getElementById('form__text');

    if(nameField.value.search("^[a-zA-Za-яА-Я ]+$") === -1)
        nameField.style.border = "1px solid red";
    else
        nameField.style.border = "none";

    if(phoneField.value.search("^{+}[7|8][(]\d{3}[)]\d{3}-\d{4}$") === -1)
        phoneField.style.border = "1px solid red";
    else
        phoneField.style.border = "none";

    if(emailField.value.search("^([A-zА-я0-9_-]+\.)*[A-zА-я0-9_-]+@[A-zА-я0-9_-]+(\.[A-zА-я0-9_-]+)*\.[A-zА-я]{2,6}$") === -1)
        emailField.style.border = "1px solid red";
    else
        emailField.style.border = "none";

    if(textField.value.search("^[a-zA-Za-яА-Я0-9_-]+$") === -1)
        textField.style.border = "1px solid red";
    else
        textField.style.border = "none";
}

document.getElementById('form__button-submit').addEventListener('click', () => {
    validateFields();
});